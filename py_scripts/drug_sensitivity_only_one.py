...
def create_report(filename, list_comments):
    with open('/hps/research1/icortes/acunha/python_scripts/Drug_sensitivity/new_results_only_one/{}'.format(filename), 'a') as f:
        f.write('\n'.join(list_comments))
...
class Drug_sensitivity_single_cell:
    ...
    def set_parameters(self, list_parameters):
        ...
        self.to_keep = list_parameters[12]
        ...
        if self.run_type == 'resume':
            self.n_epochs += int(list_parameters[19])
        ...
        if self.drug_from == 'prism':
            self.path_data = '/hps/research1/icortes/acunha/python_scripts/Drug_sensitivity/data_secondary/ic50/{}_{}/'.format(self.drug_from, self.sc_from)
        else:
            self.path_data = '/hps/research1/icortes/acunha/python_scripts/Drug_sensitivity/data/{}_{}_ic50/'.format(self.sc_from, self.drug_from)
        self.new_indexes2barcode_screen = pickle.load(open('{}/{}_{}_new_indexes_newIndex2barcodeScreen_dict.pkl'.format(self.path_data, self.drug_from, self.sc_from), 'rb'))
        ...
        if self.run_type == 'resume':
            ...
            lines = ['** REPORT - DRUG SENSITIVITY **\n',
                    ...
                    'Type of split: {} ; Keep: {}'.format(self.type_of_split, self.to_keep),
                    'Perc. of train: {}% ; Perc of validation: {}% ; Perc of test: {}% \n'.format(int(self.perc_train*100), int(self.perc_val*100), int((1-self.perc_train-self.perc_train)*100))]
    ...
    def get_indexes(self):
        if self.sc_from == 'pancancer':
            if self.type_of_split == 'only-one-cell-test-drugs':
                #celllines2indexes dict
                celllines2indexes = pickle.load(open('{}/{}_{}_new_indexes_dict.pkl'.format(self.path_data, self.drug_from, self.sc_from), 'rb'))
                final_indexes = celllines2indexes[self.to_keep]
            
            elif self.type_of_split == "only-one-drug-test-cells":
                #drug2indexes dict
                drug2indexes = pickle.load(open('{}/{}_{}_drug2indexes_dict.pkl'.format(self.path_data, self.drug_from, self.sc_from), 'rb'))
                final_indexes = drug2indexes[self.to_keep]
    ...
    def get_batches(self, sc_bottlenecks, drug_bottlenecks, indexes, type_dataset):
        data = []
        sensitivity = []
        for index in indexes:
            ...
            values = []
            if self.type_of_split == 'only-one-cell-test-drugs':
                data.append(drug_bottlenecks[screen[0]])
                sensitivity.append([sens])
                values.append((self.to_keep, screen[1], screen[0], sens))
            elif self.type_of_split == "only-one-drug-test-cells":
                barcodes = list(ccle[1].keys())
                for i in range(len(barcodes)):
                    data.append(sc_bottlenecks[ccle[1][barcodes[i]]])
                    sensitivity.append([sens])
                    values.append((self.to_keep, barcodes[i], ccle[1][barcodes[i]], ccle[0], sens))
            if type_dataset == 'Train':
                self.train_barcodes.extend(values)
            elif type_dataset == 'Validation':
                self.validation_barcodes.extend(values)
            else:
                self.test_barcodes.extend(values)
    ...
    def save_dataset(self, type_dataset):
        ...
        with open('pickle/{}_set_total.txt'.format(type_dataset), 'w') as f:
            if self.type_of_split == 'only-one-cell-test-drugs':
                f.write(','.join(['cell_kept', 'drug', 'screen_i', 'real_sensitivity', 'predicted_sensitivity']))
                for i in range(len(values)):
                    _, screen_i, _, sens = values[i].split(',')
                    if ':::' in screen_i:
                        screen = screen_i.split(':::')[0]
                    sens_list.append(str(sens))
                    f.write('\n{},{},{},{},{}'.format(self.to_keep, screen, screen_i, sens, output[i]))
            else:
                f.write(','.join(['drug_kept', 'barcode', 'cell_line', 'real_sensitivity', 'predicted_sensitivity']))
                for i in range(len(values)):
                    barcode, _, ccle, sens = values[i].split(',')
                    sens_list.append(str(sens))
                    f.write('\n{},{},{},{},{}'.format(self.to_keep, barcode, ccle, sens, output[i]))
    ...
    def __train_validation_nnet(self, model, sc_bottlenecks, drug_bottlenecks, train_set_index, validation_set_index):
        ...
        for epoch in range(start_point, self.n_epochs):
            ...
            model, optimizer, train_loss_epoch = self.__train_nnet__(model, optimizer, sc_bottlenecks, drug_bottlenecks, train_set_index, 'Train')
            ...
            validation_loss_epoch = self.__validation_and_test_nnet__(model, sc_bottlenecks, drug_bottlenecks, validation_set_index, 'Validation')
    ...
    def __train_nnet__(self, model, optimizer, sc_bottlenecks, drug_bottlenecks, train_set_index, type_dataset):
        train_loss_epoch = 0.0
        model.train()  # set model for training
        n_batches = 0
        for i in range(0, len(train_set_index), self.size_batch):
            inputs, real_values = self.get_batches(sc_bottlenecks, drug_bottlenecks, train_set_index[i:int(i + self.size_batch)], type_dataset)
    ...
    def __validation_and_test_nnet__(self, model, sc_bottlenecks, drug_bottlenecks, dataset_index, type_dataset):
        loss_epoch = 0.0
        model.eval()
        n_batches = 0
        with torch.no_grad():
            for i in range(0, len(dataset_index), self.size_batch):
                inputs, real_values = self.get_batches(sc_bottlenecks, drug_bottlenecks, dataset_index[i:int(i + self.size_batch)], type_dataset)
                inputs = inputs.to(self.device)
                real_values = real_values.to(self.device)
    ...
    def __get_predictions_nnet__(self, model, sc_bottlenecks, drug_bottlenecks, type_dataset):
        if type_dataset == 'Train':
        ...
        with torch.no_grad():
            with open('pickle/{}_output.txt'.format(type_dataset), 'w') as f:
                ...
                for i in range(len(indexes)):
                    if i+1 % self.size_batch != 0:
                        index = indexes[i]
                    
                        if self.type_of_split == 'only-one-cell-test-drugs':
                            _, _, screen_id, sens = index
                            data.append(drug_bottlenecks[int(screen_id)])
                        else:
                            _, _, barcode_id, _, sens = index
                            data.append(sc_bottlenecks[int(barcode_id)])
                        sensitivity.append([sens])
                    else:
                        inputs = torch.Tensor(data).type('torch.FloatTensor').to(self.device)
                        real_values = torch.Tensor(np.array(sensitivity)).type('torch.FloatTensor').to(self.device)
                        predictions = model(inputs)
                        f.write('\n'.join([str(x) for x in predictions.detach().cpu().numpy().tolist()]))
                        f.write('\n')
                        data = []
                        sensitivity = []
    ...
    def __train_rf_or_lgbm_or_linear(self, model, sc_bottlenecks, drug_bottlenecks, train_set_index):
        X_train = []
        y_real = []
        ...
        for index in train_set_index:
            ...
            values = []
            if self.type_of_split == 'only-one-cell-test-drugs':
                X_train.append(drug_bottlenecks[screen[0]])
                y_real.append([sens])
                values.append((self.to_keep, screen[1], screen[0], sens))
            elif self.type_of_split == "only-one-drug-test-cells":
                barcodes = list(ccle[1].keys())
                for i in range(len(barcodes)):
                    X_train.append(sc_bottlenecks[ccle[1][barcodes[i]]])
                    y_real.append([sens])
                    values.append((self.to_keep, barcodes[i], ccle[1][barcodes[i]], ccle[0], sens))
            self.train_barcodes.extend(values)
    ...
    def train_model(self, model, sc_bottlenecks, drug_bottlenecks, train_set_index, validation_set_index = []):
        start_training = time.time()

        if self.model_architecture == 'NNet':
            model = self.__train_validation_nnet(model, sc_bottlenecks, drug_bottlenecks, train_set_index, validation_set_index)

        elif self.model_architecture == 'lGBM':
            model = self.__train_rf_or_lgbm_or_linear(model, sc_bottlenecks, drug_bottlenecks, train_set_index)
        
        end_training = time.time()
        create_report(self.filename_report, ['Duration: {:.2f} \n'.format(end_training - start_training)])
        
        if self.model_architecture == 'NNet':
            self.__get_predictions_nnet__(model, sc_bottlenecks, drug_bottlenecks, 'Train')
            self.__get_predictions_nnet__(model, sc_bottlenecks, drug_bottlenecks, 'Validation')
    ...
    def __run_test_set_nnet(self, model, sc_bottlenecks, drug_bottlenecks, test_set_index):
        test_loss = self.__validation_and_test_nnet__(model, sc_bottlenecks, drug_bottlenecks, test_set_index, 'Test')
    ...
    def __run_test_set_rf_or_lgbm_or_linear(self, model, sc_bottlenecks, drug_bottlenecks, test_set_index):        
        y_real = []
        y_pred = []
        ...
        with open('pickle/Test_output.txt', 'w') as f:
            with open('pickle/Test_set_barcodes.txt', 'w') as file:
                ...
                for index in test_set_index:
                    ...
                    barcodes = list(ccle[1].keys())
                    if self.type_of_split == 'only-one-cell-test-drugs':
                        X.append(drug_bottlenecks[screen[0]])
                        y_real.append([sens])
                        file.write('{},{},{},{}\n'.format(self.to_keep, screen[1], screen[0], sens))
                    elif self.type_of_split == "only-one-drug-test-cells":
                        barcodes = list(ccle[1].keys())
                        for i in range(len(barcodes)):
                            X.append(sc_bottlenecks[ccle[1][barcodes[i]]])
                            y_real.append([sens])
                            file.write('{},{},{},{},{}\n'.format(self.to_keep, barcodes[i], ccle[1][barcodes[i]], ccle[0], sens))
    ...
    def run_test_set(self, model, sc_bottlenecks, drug_bottlenecks, test_set_index):
        if self.model_architecture == 'NNet':
            self.__run_test_set_nnet(model, sc_bottlenecks, drug_bottlenecks, test_set_index)
        elif self.model_architecture == 'lGBM':
            self.__run_test_set_rf_or_lgbm_or_linear(model, sc_bottlenecks, drug_bottlenecks, test_set_index)
   ...
   # --------------------------------------------------
    ...
    def get_drug_from(self):
        return self.drug_from
    ...
    def save_parameters(self):
        if self.model_architecture == 'NNet':
            ...
            pickle.dump([network_info, np.format_float_positional(self.learning_rate), str(self.size_batch), str(self.n_epochs), str(self.perc_train),
                         ...
                         str(self.to_keep), self.type_lr, self.data_from, self.model_architecture, self.early_stop, self.combination], open('pickle/list_initial_parameters_single_cell.pkl', 'wb'))
...
def run_drug_prediction(list_parameters, run_type):
    ...
    drug_from = drug_sens.get_drug_from()
    ...
    if self.type_of_split == 'only-one-cell-test-drugs':
        drug_bottlenecks = pd.read_csv('{}/molecular/{}_bottlenecks.csv'.format(path_data, drug_from), header = 0, index_col = 0)
        input_size = drug_bottlenecks.shape[1]
        sc_bottlenecks = []
        drug_bottlenecks = drug_bottlenecks.to_numpy()
    else:
        sc_bottlenecks = pd.read_csv('{}/single_cell/{}_{}_bottlenecks.csv'.format(path_data, sc_from, combination), header = 0, index_col = 0)
        sc_bottlenecks = sc_bottlenecks.iloc[:, :-1]
        input_size = sc_bottlenecks.shape[1]
        drug_bottlenecks = []
        sc_bottlenecks = sc_bottlenecks.to_numpy()
    
    
    #start the Drug Sensitivity model
    if model_architecture == 'NNet':
        model = drug_sens.initialize_model(size_input=input_size)
    elif model_architecture == 'lGBM':
        model = drug_sens.initialize_model(size_input=[])
    
    #train the model
    model_trained = drug_sens.train_model(model, sc_bottlenecks, drug_bottlenecks, train_set_index, validation_set_index)
    drug_sens.run_test_set(model_trained, sc_bottlenecks, drug_bottlenecks, test_set_index)
