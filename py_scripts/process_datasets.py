...
class Process_dataset_pancancer():
    ...
    def create_integrated_datasets(self, screens_list, prism_dataset, prism_bottlenecks, pancancer_bottlenecks, ccle_per_barcode, what_type):
        barcode2indexes = {}
        new_indexes_dict = {}

        for i in range(pancancer_bottlenecks.shape[0]):
            bar = pancancer_bottlenecks.iloc[i].name
            ccle = ccle_per_barcode[bar]
            ...
            for j in range(prism_bottlenecks.shape[0]):
                ...
                if not np.isnan(sens_value):
                    ...
                    if new_index not in new_indexes_dict.keys():
                        new_indexes_dict[new_index] = [((bar, i), (screen, j), sens_value)]
                    else:
                        new_indexes_dict.append(((bar, i), (screen, j), sens_value))
                    indexes.append(new_index)
            barcode2indexes[bar] = indexes

        pickle.dump(barcode2indexes, open('/hps/research1/icortes/acunha/python_scripts/Drug_sensitivity/data/prism_pancancer/prism_pancancer_new_indexes_5percell_line_{}_dict.pkl'.format(what_type), 'wb'))
        pickle.dump(new_indexes_dict, open('/hps/research1/icortes/acunha/python_scripts/Drug_sensitivity/data/prism_pancancer/prism_pancancer_new_indexes_5percell_line_{}_newIndex2barcodeScreen_dict.pkl'.format(what_type), 'wb'))
    ...
    def run(self):
        ...
        self.create_integrated_datasets(list_indexes_prism, prism_matrix, prism_bottlenecks, pancancer_bottlenecks, ccle_per_barcode, 'once')
